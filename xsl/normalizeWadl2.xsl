...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
                ...
                xmlns:raxf="http://docs.rackspace.com/functions"
    ...
    <xsl:template match="node() | @*" mode="normalizeWadl2">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:choose> 
                        <xsl:when test="(@id[parent::wadl:resource] or @id[parent::wadl:method]) and $baseID != ''">
                            <xsl:apply-templates select="node() | @*" mode="normalizeWadl2">
                                <xsl:with-param name="baseID" select="concat($baseID,'_',@id)"/>
                            </xsl:apply-templates>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:apply-templates select="node() | @*" mode="normalizeWadl2">
